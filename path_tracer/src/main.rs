...
fn main() {
    let mut camera = Camera::build_default(WIDTH, HEIGHT, 5, 500, UPSCALE);
    ...
    camera.render_to_file_par(&mut file, &scene, false, 0).expect("failed rendering image: io failure");
